 ...
 * @Author: your name
 * @Date: 2019-09-26 19:31:09
 * @LastEditTime: 2019-09-26 19:31:09
...
export function enableGesture(main) {
  ...
  let move = (point, context, origin) => {
    ...
    if (
      ...
      if (context.isPan == false) {
        ...
        if (
          Math.abs(point.clientX - context.startX) <
          Math.abs(point.clientY - context.startY)
        ) {
  ...
  let end = (point, context, origin) => {
    ...
    if (context.isPan) {
      ...
      let v =
        Math.sqrt(
          Math.pow(point.clientX - context.startX, 2) +
            Math.pow(point.clientY - context.startY, 2)
        ) / t;
      if (v > 0.3) {
        isFlick = true;
