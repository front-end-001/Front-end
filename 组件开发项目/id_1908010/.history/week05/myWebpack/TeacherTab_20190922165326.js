...
export default class TabView { //是否可以抽象出来一部分作为基类？
    ...
    didMounted() {
        ...
        let panend = (event) => {
            ...
            if (event.isFlick && Math.abs(event.dx) > Math.abs(event.dy)) {
                ...
                this[STATE_SYMBOL].position = - Math.round((this.startX + event.dx) / parentW);  // 取最近的整数
            }
    
            this[STATE_SYMBOL].position = Math.max(0, Math.min(this[STATE_SYMBOL].position, this.children.length - 1));//如果position 不取正值
    
            for (let child of children) {
                child.style.transition = "";
                child.style.transform = `translate(${-this[STATE_SYMBOL].position * parentW}px)`; //复位
            }
    
            this.startX  = -this[STATE_SYMBOL].position * parentW;
            this.didUpdate()
        }
    ...
    appendChild(child) {
        ...
        for (let i = 0; i < this.contentContainer.children.length; i ++) {
            ...
            this.contentContainer.children[i].style.vertical
        }
