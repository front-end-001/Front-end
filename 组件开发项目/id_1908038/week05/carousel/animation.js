...
class TimeLine {
    ...
    start() {
        ...
        requestAnimationFrame(this._tick); //1000/60=16
    }
    restart() {
        if (this._tick) {
            this._tick = null;
    ...
    tick(t) { //标志
        if (t > this._endTime) {
            ...
            if (!this._fixKeyFrame)
                ...
                t = this._startTime;
                this._fixKeyFrame = false; //最后一帧不修复
            }
        } else {
        ...
        // let displacement = Math.sin(progress * Math.PI / 2) * (this._endValue - this._startValue);
        let currentValue = this._startValue + displacement;
        this._element.style[this._property] = this._converter(currentValue); //先行差值
        // this._element.style[this._property] = this._converter((t - this._startTime) / (this._endTime - this
        //   ._startTime) * (this._endValue - this._startValue) + this._startValue); //先行差值
    }
}
