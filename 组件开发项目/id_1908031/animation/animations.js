...
class Timeline {
	// 初始化-inited，开始运行-started，暂停-paused
	constructor() {
		this._animations = [];
		this.status = "inited"
		this._rate = 1
	...
	resume() {
		...
		this.status = "started" // 标注一下，这个注释写下时视频没有这一句
	...
	start() {
		...
		this._tick = () => {
			...
			// 如果使用setTimeout，那么在这里需要在触发一次,并且对应的clearInterval要改为clearTimeout
			// this._timer = setTimeout(this._tick, 16)
			// 进一步优化
			if (this._tick) {
				requestAnimationFrame(this._tick)
...
export { Timeline, DOMElementStyleVectorAnimation, DOMElementStyleNumberAnimation, cubicBezier };
